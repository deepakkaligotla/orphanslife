import { StyleSheet, Text, View, TextInput, ScrollView, Pressable, Dimensions, Modal, TouchableOpacity, FlatList } from "react-native";
      ...
      <>
        ...
        <ScrollView>
          <View style={styles.container}>
            ...
            <View>
              <Modal visible={this.state.personModal}
                transparent
                animationType="slide"
                onRequestClose={() => this.setState({ personModal: false })}>
                <TouchableOpacity
                  style={styles.overlay}>
                  <View style={[styles.dropdown, { top: this.state.dropdownTop }]}>
                    <FlatList
            ...
            <View style={{ flexDirection: 'row-reverse', borderColor:'white', borderWidth:1, width:150, marginTop:10 }}>
             <Pressable
                onPress={() => this.setState({ personModal: true })}>
                <Icon size={30} color="white" name="circledown" />
              </Pressable>
              <Text style={{ color: 'white', marginRight:10, fontSize:20 }}>{this.state.selectedPerson}</Text>
            </View>
            
            {/* Selecting Admin_role dropdown */}
            <View>
              <Modal visible={this.state.adminRoleModal}
                transparent
                animationType="slide"
                onRequestClose={() => this.setState({ adminRoleModal: false })}>
                <TouchableOpacity
                  style={styles.adminRoleoverlay}>
                  <View style={[styles.adminRoleDropdown, { top: this.state.adminRoleDropdownTop }]}>
                  <FlatList
                      data={this.state.roles}
                      renderItem={({ item }) => (
                        <TouchableOpacity style={styles.adminRoleitem} onPress={() => {
            ...
            <View style={{ flexDirection: 'row-reverse', borderColor:'white', borderWidth:1, width:150, marginTop:10 }}>
             <Pressable
                onPress={() => this.setState({ adminRoleModal: true })}>
                <Icon size={30} color="white" name="circledown" />
              </Pressable>
              <Text style={{ color: 'white', marginRight:10, fontSize:20 }}>{this.state.selectedAdminRole}</Text>
            </View>
            ...
            <View>
              <Modal visible={this.state.genderModal}
                ...
                <TouchableOpacity
                  ...
                  <View style={[styles.genderDropdown, { top: this.state.genderDropdownTop }]}>
                  <FlatList
